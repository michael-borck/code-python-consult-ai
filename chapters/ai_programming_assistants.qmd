...
- recognise when AI assistance is most (and least) helpful
...
Complete code examples and notebooks for all chapters are available on GitHub with "Open in Colab" buttons. See [books.borck.education](https://books.borck.education).
:::
...
- **optimisation**: "optimise this code that's calculating Fibonacci numbers"
...
2. AI programming assistants are particularly well-suited for which of these tasks?
   ...
   d) optimising code when microsecond performance matters
...
response_manager.py: Manages response generation
history_manager.py: Handles conversation history
pattern_matcher.py: Identifies intents from input

Classes
-------
Chatbot: Main chatbot interface
PatternMatcher: Analyses user input
ResponseGenerator: Creates context-appropriate responses
HistoryManager: Stores conversation history
...
- Previous Chapter: [Getting Help](getting_help.qmd)
