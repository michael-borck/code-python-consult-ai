...
- recognise the importance of fundamental Python knowledge in the AI era
...
- You make the important decisions about design, optimisations, and approaches
...
```text
AI: Here's a program that displays the current
CPU temperature:

import psutil
temp = psutil.sensors_temperatures()
print(f"CPU temp: {temp['coretemp'][0].current}°C")
```
...
```text
AI: I see! Here's a program that checks the
...
print(f"Temperature in {city}: {get_weather(city)}°C")
```
...
```text
Human: I keep getting an IndexError when trying
to run this code:
...
```text
AI: The issue is in your loop. You're accessing
data[i+1], but when i equals the last index (2),
you'll try to access data[3], which doesn't exist.
...
Use AI to quickly generate a working prototype, then thoroughly review, refactor, and customise it to meet your needs and standards.
...
```text
Write a Python function that saves a dictionary
to a JSON file with proper error handling. The
function should take three parameters: the
dictionary to save, the filename, and an optional
parameter to format the JSON with indentation.
```
...
- Next Chapter: [Getting Started with Python](getting_started.qmd)
- Related Topics: [Working with AI Assistants](working_with_ai_assistants.qmd)
