...
- Apply common string methods to transform and analyse text
...
Strings are one of Python's most versatile and commonly used data types. Whether you're building a web application, analysing data, creating a chatbot, or just printing information to users, text manipulation is essential. Python provides a rich set of tools for working with strings, making tasks that would be complex in other languages straightforward and intuitive.
...
If you worked through *Think Python, Direct AI*, you already used string methods like `.lower()` and `.split()` to process user input in your projects. Here we explore strings more thoroughly — how they work internally, the full range of methods available, and when to reach for each one.
:::
...
c = "he" + "llo"
print(a is c)  # May be True due to compiler optimisation
...
- Create a "summarise" command that extracts key information from longer text
...
- **Previous Chapter**: [Going Loopy](going_loopy.qmd) - Learn how to use loops, which are often used to process strings
- **Next Chapter**: [Dictionaries](dictionaries.qmd) - Key-value pairs that can be used to store and retrieve text
- **Related Topics**:
  - [Lists](lists.qmd) - Another sequence type with many similarities to strings
  - [Input and Output](input.qmd) - Reading and writing text is fundamental to programs
  - [Error Handling](errors_and_exceptions.qmd) - Handling potential errors in string operations
...
2. **Text Analysis**: Counting words, extracting keywords, and analysing sentiment.
   ```python
   text = "Python is amazing and powerful!"
   word_count = len(text.split())  # 5 words
   ```

3. **Template Generation**: Creating customised documents, emails, or web content.
   ```python
