...
Code examples and notebooks for all chapters are available on GitHub with "Open in Colab" buttons. See [books.borck.education](https://books.borck.education).
:::
...
- **organise** your code into logical, self-contained units
...
This behaviour explains why `print()` doesn't appear to return anything - it's returning `None` after displaying output.
...
Now let's apply these principles to enhance our chatbot project with well-designed functions. Previously, we had a simple script; now we'll organise it into modular, reusable components.
...
This modular approach makes extending your chatbot clean and manageable. Each new capability goes into its own function, keeping your code organised.
...
2. analyses the text for positive and negative keywords
...
**AI Response:**
*Here's a well-designed function to analyse sentiment in chatbot messages:*

```python
def analyze_sentiment(message, custom_keywords=None):
    """
    analyse the sentiment of a message as positive, negative, or neutral.

    Args:
        message (str): The user message to analyse
...
1. *Function signature:*
   - *Primary parameter (`message`) for the text to analyse*
   - *Optional parameter (`custom_keywords`) for flexibility/customisation*
...
3. *Well-structured implementation:*
   ...
   - *Default keywords with option for customisation*
...
```python
# Confusing name - doesn't match behaviour
...
def calculate_damage(attacker, defender, attack_type="normal"):
    ...
    Args:
        ...
        defender (dict): Defender stats including 'defence', 'level', 'resistance'
    ...
    # Apply defender's defence
    defense_reduction = defender.get('defence', 0) * 0.5
...
5. Which of the following is the best practice for function design?
   ...
   d) minimising the number of functions in your program
...
- **Previous Chapter**: [Functions](using_functions.qmd) — Learn how to use built-in and library functions
- **Next Chapter**: [Making Decisions](making_decisions.qmd) — Control program flow with if statements
- **Related Topic**: [Modules and Packages](modules_and_packages.qmd) — organise your functions into reusable modules
- **Project Connection**: [Testing](testing.qmd) — Learn how to verify your functions work correctly
- **Advanced Application**: [Object-Oriented Programming](orientating_your_objects.qmd) — Combine functions and data into classes
