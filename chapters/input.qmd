...
This behaviour is consistent but can cause confusion, especially with numeric input:
...
travelers = int(input("How many people are travelling? "))
...
2. **Describe the user experience** you want to create (error messages, retry behaviour)
...
- **Previous Chapter**: [Output](output.qmd) — Learn how to display information to users
- **Next Chapter**: [Operators](operators.qmd) — Manipulate data with mathematical and logical operations
- **Related Topic**: [Error Handling](errors_and_exceptions.qmd) — Learn to handle invalid input gracefully
- **Project Connection**: [Making Decisions](making_decisions.qmd) shows how to use input to drive program flow
- **Advanced Application**: In [Python AI Integration](python_ai_integration.qmd), you'll use input to interact with AI services
