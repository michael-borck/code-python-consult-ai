...
Notice that `print()` automatically adds spaces between multiple items. This behaviour can be customised, as we'll see shortly.
...
Please show code examples for each of these needs, optimised for readability on a terminal interface.
```
...
- **Previous Chapter**: [Variables](variables.qmd) — Learn how to name and store your data
- **Next Chapter**: [Input](input.qmd) — Get information from users into your programs
- **Related Topic**: [String Manipulation](strings.qmd) — Master text processing techniques
- **Project Connection**: See how our output system evolves in [Dictionaries](dictionaries.qmd) to handle complex message templates
- **Advanced Application**: In [Python AI Integration](python_ai_integration.qmd), we'll use these output techniques to display AI-generated responses
