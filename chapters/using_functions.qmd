...
- Track the words the user has used, in the order they first appeared
- Use a "words" command to show the 5 most recent unique words
...
- Track every unique word the user has used in a list, keeping the words in the order they first appeared (skip a word if it is already in the list)
- For a "words" command, show the 5 most recently added unique words using list slicing
...
`.split()` breaks a string into a list of words. `len(words)` counts them. `unique_words` starts as an empty list. The `not in` check (Chapter 7's membership operator) appends each word only the first time it appears, so `unique_words` keeps the words in the order you first used them. That order is what makes `[-5:]` mean "the 5 most recent" — slicing a list keeps its order. `max(message_count, 1)` prevents division by zero.
:::

### Stretch It

::: {.callout-note title="Building Session Prompt"}
Add an "info" command that takes a word as argument (e.g., "info python"). Use len() to show the word's character count, and check if it is in the unique_words list.
:::
