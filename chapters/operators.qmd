...
Operators are the essential tools that allow you to manipulate data, make decisions, and create dynamic behaviour in your programs. Just as physical tools transform raw materials into finished products, operators transform raw data into meaningful information and actions.
...
This behaviour is particularly useful for conditional execution and validation:
...
### optimisation with Compound Assignment

```python
def analyze_text(text):
    """analyse text for character distributions and statistics."""
    # Initialize counters
    char_count = 0
    word_count = 0
    line_count = 0
...
- analyses whether it's a question (ends with ?)
...
- recognise at least 5 different question types using operators
...
- **Previous Chapter**: [Input](input.qmd) — Learn how to collect information from users
- **Next Chapter**: [Functions](using_functions.qmd) — organise code into reusable blocks
- **Related Topic**: [Making Decisions](making_decisions.qmd) — Expand on the conditional logic introduced here
- **Project Connection**: [Lists](lists.qmd) — Learn how operators work with list data structures
- **Advanced Application**: [Testing](testing.qmd) — See how comparison operators are used in test assertions
...
In the next chapter, we'll explore functions—the next level of code organisation that will help us structure our chatbot's capabilities into reusable, modular components.
