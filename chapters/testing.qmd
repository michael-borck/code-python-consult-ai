...
- Use assertions to check code behaviour
...
Our implementation is already pretty efficient with the `n**0.5` optimisation, but we might add some comments or clearer variable names if needed.
...
1. **Keep tests small and focused**: Each test should verify one specific behaviour
...
4. **organise tests logically**: Group related tests into classes or modules
5. **Test both positive and negative cases**: Check that errors are handled correctly
6. **Avoid testing implementation details**: Test behaviour, not how it's implemented
...
- Previous Chapter: [Debugging](debugging.qmd)
- Next Chapter: [Modules and Packages](modules_and_packages.qmd)
